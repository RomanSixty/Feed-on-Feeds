...
?>
        <option value=""<?php if (empty($sidebar_style)) {
	echo ' selected';
}
?>>Default</option>
        <option value="simple"<?php if ($sidebar_style === 'simple') {
	echo ' selected';
}
?>>Simple</option>
        <option value="fancy"<?php if ($sidebar_style === 'fancy') {
	echo ' selected';
}
?>>Fancy</option>
    </select>
    <br>
    Default display order: <select name="order"><option value=desc>new to old</option><option value=asc <?php if ($prefs->get('order') == "asc") {
	echo "selected";
}
...
?>>
    <br>
    Use keyboard shortcuts? <input type="checkbox" name="keyboard"<?php if ($prefs->get('keyboard')) {
	echo " checked=true";
}
?>>
    <br>
    Open articles in new window? <input type="checkbox" name="item_target"<?php if ($prefs->get('item_target')) {
	echo " checked=true";
}
?>>
    <br>
  ...
  <fieldset>
    ...
    Share <select name="sharing">
      ...
      <option value="all"<?php if ($prefs->get('sharing') == 'all') {
	echo ' selected';
}
?>>all</option>
      <option value="tagged"<?php if ($prefs->get('sharing') == 'tagged') {
	echo ' selected';
}
?>>tagged as "shared"</option>
      <option value="all_tagged"<?php if ($prefs->get('sharing') == 'all_tagged') {
	echo ' selected';
}
?>>all tagged items</option>
    </select> items.<?php if ($prefs->get('sharing') != "no") {
	echo " <small><i>(your default shared page is <a href='./shared.php?user=" . urlencode($fof_user_id) . "'>here</a>)</i></small>";
}
?>
    <br>
...
<br>
<?php foreach (fof_get_plugin_prefs() as $plugin_pref) {
	$name = $plugin_pref[0];
	$key = $plugin_pref[1];
	$type = $plugin_pref[2];?>
<?php echo $name ?>:
